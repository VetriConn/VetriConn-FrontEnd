...
describe("Signup Validation Schemas", () => {
  ...
  describe("Step 2: Create Account Schema", () => {
    ...
    it("should reject empty full name", () => {
      const result = step2Schema.safeParse({ ...validData, full_name: "" });
      expect(result.success).toBe(false);
    });

    it("should reject full name with less than 2 characters", () => {
      const result = step2Schema.safeParse({ ...validData, full_name: "A" });
      expect(result.success).toBe(false);
    });
    ...
    it("should reject when passwords do not match", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message.includes("do not match")),
        ).toBe(true);
      }
    });
  });
